	...
	const int cur_ptr)
{
	...
	if(min_distance < distance_threshold_)
	{
		...
		RCLCPP_DEBUG(rclcpp::get_logger("lidar_iris_descriptor"), "\033[1;33m[Iris Intra Loop<%d>] btn %d and %d. Dis: %.2f.\033[0m",
			id_, cur_ptr, min_index, min_distance);
	}
	else
	{
		RCLCPP_DEBUG(rclcpp::get_logger("lidar_iris_descriptor"), "\033[1;33m[Iris Intra Not loop<%d>] btn %d and %d. Dis: %.2f.\033[0m",
			id_, cur_ptr, min_index, min_distance);
	}
	return result;
}
	...
	const int cur_ptr)
{
	...
	if(min_distance < distance_threshold_)
	{
		result.first = min_index;
		result.second = min_bias;
		RCLCPP_DEBUG(rclcpp::get_logger("lidar_iris_descriptor"), "\033[1;33m[Iris Inter Loop<%d>] btn %d-%d and %d-%d. Dis: %.2f. Bias:%d\033[0m", id_,
	...
	{
		RCLCPP_DEBUG(rclcpp::get_logger("lidar_iris_descriptor"), "\033[1;33m[Iris Inter Not loop<%d>] btn %d-%d and %d-%d. Dis: %.2f. Bias:%d\033[0m", id_,
			iris_feature_index_pairs[cur_ptr].first, iris_feature_index_pairs[cur_ptr].second,
			iris_feature_index_pairs[min_index].first, iris_feature_index_pairs[min_index].second, min_distance, min_bias);
	}
	return result;
